...
# MUST stay at 1 worker (ZET-6). ConnectionManager keeps live WebSocket
# connections in a per-process dict (app.state.comms), so it is NOT shared
# across workers. With >1 worker, a message or presence update only reaches
# users who happen to share the sender's worker — delivery silently breaks
